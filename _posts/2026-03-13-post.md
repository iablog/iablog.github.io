...
O modelo precisa de uma descrição precisa do ambiente, pois sem isso ele não consegue interpretar o mapa. E o formato de saída é rígido: `THOUGHT:` na primeira linha, `ACTION:` na segunda. Essa rigidez permite que o código extraia o pensamento e a ação da resposta de forma confiável com uma expressão regular simples.
...
def extrair_pensamento_e_acao(texto_resposta):
    match_pensamento = re.search(r"THOUGHT: (.*)", texto_resposta)
    str_pensamento = match_pensamento.group(1) if match_pensamento else "(não encontrado)"

    match_acao = re.search(r"ACTION: (GIRA_ANTI_HORARIO|GIRA_HORARIO|FRENTE)", texto_resposta)
    if match_acao:
