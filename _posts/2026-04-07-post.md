...
Neste artigo, vamos mostrar um exemplo de agente CodeAct que gera um bloco de código Python, depois executa este código. Em seguida, o agente recebe como observação tudo o que tiver sido enviado para a saída padrão (`stdout`), ou seja, tudo que for impresso com `print()`. Este ciclo se repete até concluir a tarefa assim:

```
Escreve código → Executa → Captura saída padrão → Envia como observação ao agente → Repete
```

Em uma única rodada, no código gerado, o agente pode chamar várias ações, usar condicionais ou criar loops. Toda a lógica de programação fica disponível. 

Porém, o agente deve imprimir as próprias observações: se ele não chamar `print()` no código gerado, não vai "observar" nada no próximo turno.


## Tarefa de Exemplo: MiniGrid

Para demonstrar o CodeAct na prática, vamos usar novamente o **MiniGrid**, uma biblioteca que cria ambientes simples. Ela são como mini-jogos onde um agente precisa navegar por um mapa em forma de *grade retangular* para atingir um objetivo dado.

![Animação do Minigrid](/assets/img/post_03_13/meu_grid_animado.gif)

No MiniGrid, o agente pode se mover para frente, girar 90 graus para cada lado, abrir portas e fazer mais algumas ações (que não vamos usar). Cada episódio tem uma missão textual, como: *"chegar ao objetivo evitando a lava"*. 
